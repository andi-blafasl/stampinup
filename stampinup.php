...
Description: This Plugin adds a shortcode for easy Stampin Up Product presentation and adds your demoid to every stampinup shop link to you place in the content.
Author: Andreas H.
Version: 0.5
...
function stempeltier_stampused_sc( $atts , $content = null ) {
    ...
    foreach ($products as $product) {
        ...
        if ( preg_match( "/^[0-9]{6}$/", $product) ) {
            ...
            $return.= "    <a href=\"https://www.stampinup.de/products/$product?demoid=$demoid$hostcode\" 
            ...
            /*$return.= "    <a href=\"http://www2.stampinup.com/ECWeb/ProductDetails.aspx?productID=$product&demoid=$demoid\" 
                alt=\"Stampin Up Product $product\" title=\"Stampin Up Online Shop Product $product\" target=\"_blank\">
                <img src=\"//www2.stampinup.com/images/EC/$product$imgsize.jpg\" alt=\"Stampin Up Product $product\"></a>";*/
            $return.= "  </div>\n";
        }
...
//    if( is_singular() && is_main_query() ) {
    if( is_main_query() ) {
        ...
        $new_content = preg_replace( '/(|\?|\&amp\;)demoid=[0-9]+/i', '', $content );
        $new_content = preg_replace( '/(href=.http:\/\/www2\.stampinup\.com\/ECWeb\/[a-z.?&=;0-9\/-]+)/i', '$1&demoid='.$demoid, $new_content) ;
		$new_content = preg_replace( '/(href=.https:\/\/www\.stampinup\.(com|de)\/search\/[a-z.?&=;0-9\/-]+)/i', '$1?&demoid='.$demoid.$hostcode, $new_content) ;
		$new_content = preg_replace( '/(href=.https:\/\/www\.stampinup\.(com|de)\/products\/[a-z.?&=;0-9\/-]+)/i', '$1?&demoid='.$demoid.$hostcode, $new_content) ;
        $content = $new_content;   
    }   
    return $content;
}
